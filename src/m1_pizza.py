...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(125.0, 90.0),  # All numbers are approximate.
                rg.Point(140.0, 75.0),
                rg.Point(125.0, 60.0),
                rg.Point(110.0, 75.0)]
    circle = rg.Circle(rg.Point(125, 75), 15)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 1:  1000 slices.'
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('POLYGON test 3:  42 segments with medium orange lines')
    window = rg.RoseWindow(350, 350, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 2
    polygon(window, circle, 7, 'orange', 4)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    # Test 3 (on another window):
    title = ('FANCY POLYGON test 3:  144 lime sienna lines on orchid circle,'
             + ' hops = 42.')
    window = rg.RoseWindow(300, 300, title)
    circle = rg.Circle(rg.Point(150, 150), 130)
    circle.fill_color = 'orchid3'
    fancy_polygon(window, circle, 144, 42, 'sienna', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(list)):
        line = rg.Line(list[k - hops_to_next_point], list[k])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
